...
const {
  insertInnerToken
} = require('./utils/insertInnerToken');
...
} = require('./lib/mysql/getConnectionPromise');
const {
  beginTransaction
} = require('./lib/mysql/beginTransaction');
...
function init() {
  aelf.chain.getChainStatus().then(chainInfo => {
    ...
    console.log('getChainInformation:', chainInfo, chainInfo.ChainId, typeof result);
...
async function subscribe(pool, scanLimit) {
  ...
  let lastestBlockInDatabaseUnconfirmed = await queryPromise(pool, 'select block_height from blocks_unconfirmed ORDER BY block_height DESC limit 1');
...
async function insertBlockAndTxs(option) {
  ...
  let insertResourceTranPromise = insertResourceTransactions(transactionsDetail, connection, 'resource' + suffix, contractAddressList);
