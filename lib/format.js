...
function transactionFormat(transaction, blockInfo, contractAddressList) {
    ...
    // 这一套规则是针对token合约的。
    const methodCheck = ['Initialize', 'Transfer', 'InitialBalance'].includes(txInfo.Method);
    if (txInfo.To === contractAddressList.token && methodCheck) {
        const params = txInfo.params.split(', ');
        switch (method) {
            case 'Initialize':
