 ...
 * 2018.08
 */
const deserializeParams = require('./protobuf/deserializeParams');
    ...
    if (txInfo.To === contractAddressList.resource) {
        ...
        switch (method) {
            case 'Initialize':
                ...
                output.params = JSON.stringify(deserializeParams(txInfo.Params, txInfo.To));
                break;
        }
    }
    else if (txInfo.To === contractAddressList.token && tokenMethodCheck) {
        // WebApi 不再返回 proto序列化的数据，返回反序列化后的json数据。
        // const params = deserializeParams(txInfo.Params, txInfo.To, {
        //     method
        // });
        ...
        switch (method) {
            case 'Create':
                ...
                // output.quantity = params.totalSupplyStr || 0;
                output.quantity = params.totalSupply || 0;
                break;
            case 'Transfer':
                output.params_to = params.to || 'tx failed';
                // output.quantity = params.amountStr || 0;
