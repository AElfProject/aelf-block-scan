...
function insertTransactions(transactions, connection, table) {
    ...
    transactions.map(item => {
        ...
        // 初始化Token的步骤变了， move to index.js
...
function insertResourceTransactions(transactions, connection, table, contractAddressList) {
    ...
    resourceTxs.map(item => {
        ...
        const type = paramsArray[0].trim();
        const quantity = parseInt(paramsArray[1].trim(), 10);
        values = values.concat([tx_id, method, type, quantity, chain_id, block_height, tx_status, time]);
