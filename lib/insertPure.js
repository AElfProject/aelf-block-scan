...
function insertBlock(blockInfoFormatted, connection, table) {
    ...
    let sql = `insert into ${table} ${keysStr} VALUES ${valuesStr} ON DUPLICATE KEY UPDATE block_hash=VALUES(block_hash);`;
...
function insertContract(transaction, connection, table) {
    ...
    let values = [transaction['address_to'], transaction['chain_id'], transaction['block_hash'], transaction['tx_id'] ].concat(params);

    let sql = `insert into ${table} ${keysStr} VALUES ${valuesBlankStr} ON DUPLICATE KEY UPDATE contract_address=VALUES(contract_address);`;
    return queryPromise(connection, sql, values);
}