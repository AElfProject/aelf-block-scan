...
function formatResourceDetail(input) {
    ...
    const tradeDetail = eventsDeserialized.find(item => {
        return item.name === 'tokenTrade';
    });
    ...
    let output = {
        ...
        type: paramsDeserialized.json.symbol || '',
        resource: parseInt(paramsDeserialized.json.amountStr || 0, 10),
